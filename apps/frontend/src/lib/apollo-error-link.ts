...
export function createRetryLink(config: RetryConfig = DEFAULT_RETRY_CONFIG) {
        ...
        forward(operation).subscribe({
          ...
          error: (error) => {
            if (shouldRetry(error, retryCount, config.max)) {
              ...
              console.log(
                `Retrying operation ${operation.operationName} (attempt ${retryCount}/${config.max})`,
              );
...
function shouldRetry(error: any, retryCount: number, maxRetries: number): boolean {
  ...
  if (error.graphQLErrors?.length > 0) {
    ...
    return error.graphQLErrors.some((gqlError: GraphQLError) =>
      retriableErrorCodes.includes(gqlError.extensions?.code as string),
    );
  }
  ...
  if (error.networkError) {
    ...
    return retriableNetworkErrors.some((errorType) =>
      error.networkError.message?.includes(errorType),
    );
  }
