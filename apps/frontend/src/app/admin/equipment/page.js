          ...
          <Card>
            ...
            <CardContent>
              <div className="flex gap-4 mb-4">
                ...
                <select className="px-3 py-2 border rounded-md" value={selectedCategory || ''} onChange={(e) => setSelectedCategory(e.target.value || null)}>
              ...
              <Table>
                ...
                <TableBody>
                  {loading ? (<TableRow>
                    ...
                    </TableRow>) : equipment.length === 0 ? (<TableRow>
                      <TableCell colSpan={6} className="text-center">
                        No equipment found
                      </TableCell>
                    </TableRow>) : (equipment.map((item) => (<TableRow key={item.id}>
                        <TableCell>
                          <div>
                        ...
                        <TableCell>
                          {item.vendorPricing?.length > 0 ? (<div className="flex items-center gap-2">
                              <DollarSign className="h-4 w-4 text-green-600"/>
                              <span className="text-sm">
                                {item.vendorPricing.length} vendor{item.vendorPricing.length > 1 ? 's' : ''}
                              </span>
                              {item.lowestVendorPrice && (<Badge variant="secondary" className="text-green-600">
                                  ${item.lowestVendorPrice.toLocaleString()}
                                </Badge>)}
                            </div>) : (<span className="text-sm text-muted-foreground">No vendor pricing</span>)}
                        </TableCell>
                        <TableCell>
              ...
              {/* Pagination */}
              {equipmentData?.equipment && (<div className="flex items-center justify-between mt-4">
                  <p className="text-sm text-muted-foreground">
              ...
              <Table>
                ...
                <TableBody>
                  {categories.map((category) => (<TableRow key={category.id}>
                      ...
                      <TableCell>
                        {equipment.filter((e) => e.category.id === category.id).length}
                      </TableCell>
                      <TableCell>
                        <div className="flex gap-1">
                          <Button variant="ghost" size="sm">
