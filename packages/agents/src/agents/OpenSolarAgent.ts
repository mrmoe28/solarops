...
import { AgentType, AgentResult, SolarDesign, BillOfMaterials } from '@solarops/shared';
...
export class OpenSolarAgent extends BaseAgent {
  ...
  async execute(input: OpenSolarInput): Promise<AgentResult<Partial<SolarDesign>>> {
    return this.executeWithRetry(async () => {
      ...
      const solarDesign: Partial<SolarDesign> = {
        systemSize,
        panelCount,
  ...
  private calculateSystemSize(squareFootage: number): number {
