...
export class PermitOfficeAgent extends BaseAgent {
    ...
    async findPermitOffice(page, input) {
        ...
        const links = await page.$$eval('a', (elements) => elements
    ...
    async scrapePermitInfo(page, url) {
        await page.goto(url);
        await page.waitForLoadState('networkidle');
        ...
        const feeTexts = await page.$$eval('*:has-text("fee"), *:has-text("cost")', (elements) => elements.map((el) => el.textContent));
        if (feeTexts.length > 0) {
            permitInfo.permitFees = { raw: feeTexts };
        }
        // Try to find requirements
        const requirementTexts = await page.$$eval('*:has-text("requirement"), *:has-text("required")', (elements) => elements.map((el) => el.textContent));
        if (requirementTexts.length > 0) {
            permitInfo.requirements = { raw: requirementTexts };
        }
        // Try to find application links
        const applicationLinks = await page.$$eval('a:has-text("application"), a:has-text("apply")', (elements) => elements.map((el) => el.href));
